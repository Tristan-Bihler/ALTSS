...
class TestAcDaempfer:
    ...
    def test_ausgangsamplitude(self, ac_daempfer_raw: SimulationResult):
        """Ausgangs-RMS muss ~3.54 V betragen (5Vp / sqrt(2))."""
        rms_out = berechne_rms(ac_daempfer_raw.signals["V(out)"])
        # 5Vp Sinus -> RMS = 5 / sqrt(2) = 3.535 V
        # Toleranz +/-10% wegen Transistor Vbe-Abfall und Arbeitspunkt
        assert rms_out == pytest.approx(5.0 / np.sqrt(2), rel=0.10), (
            f"Ausgangs-RMS = {rms_out:.3f} V, erwartet ~3.54 V"
        )

    def test_daempfungsverhaeltnis(self, ac_daempfer_raw: SimulationResult):
        """Daempfung V(out)/V(vin) muss zwischen 0.4 und 0.6 liegen."""
        rms_in  = berechne_rms(ac_daempfer_raw.signals["V(vin)"])
        rms_out = berechne_rms(ac_daempfer_raw.signals["V(out)"])
        verhaeltnis = rms_out / rms_in
        assert 0.40 <= verhaeltnis <= 0.60, (
            f"Daempfung = {verhaeltnis:.3f}, erwartet 0.4..0.6"
        )
    ...
    def test_eingangsamplitude_unveraendert(self, ac_daempfer_raw: SimulationResult):
        """Eingangs-RMS muss ~7.07 V betragen (10Vp unveraendert)."""
        rms_in = berechne_rms(ac_daempfer_raw.signals["V(vin)"])
        assert rms_in == pytest.approx(10.0 / np.sqrt(2), rel=0.05), (
